...
class LLMService: ObservableObject {
    ...
    // MARK: - Personal Daily Note Population (Rami template)

    /// Extracts structured values from a transcript into the fixed Rami daily note JSON schema.
    /// Every template field has exactly one JSON key. Use null for fields not mentioned.
    func populateRamiDailyNote(transcript: String, date: Date = Date()) async throws -> RamiDailyNoteResponse {
        Logger.ai.info("Starting Rami daily note population. Transcript: \(transcript.count) chars")
        ...
        Extract values into the JSON schema. Output valid JSON only.
        """
        ...
        let requestBody: [String: Any] = [
            ...
            "response_format": RamiDailyNoteSchema.openAIResponseFormat,
            "temperature": 0
        ]

        return try await executeRequest(requestBody: requestBody, responseType: RamiDailyNoteResponse.self)
    }

    // MARK: - Template Population (Generic)
